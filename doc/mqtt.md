# MQTT v5.0 Conformance Requirements (Appendix B)

## Definitions and Test Notes (Concise)

The statements below summarize terminology and protocol semantics that make the
Appendix B requirements easier to test without reproducing the full spec.
They are paraphrased from MQTT v5.0.

- Normative keywords: `MUST`, `MUST NOT`, `SHOULD`, `MAY`, etc. are interpreted
  per RFC 2119 (unless explicitly marked non-normative).
- Network Connection: ordered, lossless, bidirectional byte stream connecting a
  Client to a Server.
- Client/Server: a Client initiates the connection and uses MQTT; the Server
  accepts connections and manages sessions and subscriptions.
- Application Message: the logical message carried by MQTT; includes payload,
  QoS, properties, and Topic Name.
- MQTT Control Packet: one of the 15 packet types (CONNECT..AUTH) exchanged on
  the Network Connection.
- Malformed Packet vs Protocol Error: malformed cannot be parsed; protocol
  error is parseable but violates rules/state. Error handling is defined in
  section 4.13.
- UTF-8 Encoded String: length‑prefixed UTF‑8 string; MUST be well‑formed, MUST
  NOT include U+0000, and MUST NOT skip BOM (0xEF 0xBB 0xBF). Disallowed Unicode
  control/noncharacter code points SHOULD NOT appear and MAY be treated as
  malformed.
- Topic Name: UTF‑8 string without wildcards used in PUBLISH. Topic Filter:
  UTF‑8 string in SUBSCRIBE; may include wildcards; matching rules are defined
  in section 4.7.
- Packet Identifier: non‑zero 16‑bit value used for SUBSCRIBE/UNSUBSCRIBE and
  PUBLISH with QoS>0; must be unused while in flight; responses must echo the
  identifier.
- Remaining Length: variable byte integer encoding the length of Variable
  Header + Payload.
- Properties: a length‑prefixed list; property order must be preserved when
  forwarding User Properties.
- QoS: 0 (at most once), 1 (at least once, PUBACK), 2 (exactly once,
  PUBREC/PUBREL/PUBCOMP).
- Will Message: Application Message stored at CONNECT and published on abnormal
  disconnect unless cleared by normal DISCONNECT or by a new connection before
  Will Delay elapses.
- Topic Alias: Topic Name can be elided only when alias is set; zero‑length
  Topic Name without alias is a protocol error.

### Testability Notes (Server‑Side, Single Broker)

- For each `MUST`/`MUST NOT` requirement, tests should assert either compliance
  or the specified error behavior (disconnect, reason code, or silent discard).
- Where the spec allows `MAY`, tests should only assert that the behavior is
  permitted, not required.
- When a requirement references “Protocol Error” or “Malformed Packet”, tests
  should assert disconnection and any mandated reason code per section 4.13.
- For “forwarding” requirements (e.g., User Property order), tests should check
  that outbound packets preserve ordering and values.
