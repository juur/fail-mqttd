...
//static int global_raft_client_fd = -1;
...
static const unsigned  RAFT_PING_DELAY = 250;
static const unsigned  RAFT_MIN_ELECTION = RAFT_PING_DELAY * 10;
static const unsigned  RAFT_MAX_ELECTION = RAFT_MIN_ELECTION * 2;
...
static _Atomic int raft_active_clients = 0;
...
//static   in_port_t   opt_raft_client_port = 0;
...
//static struct in_addr opt_raft_client_listen;
...
static int raft_log_append(raft_log_t event, ...);
...
static int parse_cmdline_host_list(const char *tmp, struct raft_host_entry **list)
{
    char *ptr, *ptr2;
...
//                            if (opt_raft_client_port == 0)
//                                opt_raft_client_port = 1700 + opt_raft_id;
                            break;
                        case RAFT_ENABLE:
                            opt_raft = true;
                            break;
                        case RAFT_PORT:
                            ...
                            break;
#if 0
                        case RAFT_CLIENT_PORT:
                            if (value == NULL)
                                goto shit_usage;
                            tmp = atoi(value);
                            if (tmp == 0 || tmp > USHRT_MAX)
                                goto shit_usage;
                            opt_raft_client_port = tmp;
                            break;
#endif
                        case RAFT_BIND:
                            if (value == NULL)
                            ...
                            break;
#if 0
                        case RAFT_CLIENT_BIND:
                            if (value == NULL)
                                goto shit_usage;
                            if (inet_pton(AF_INET, value, &opt_raft_client_listen) == -1)
                                goto shit_usage;
                            break;
#endif
                        case RAFT_CLUSTER_ADDRS:
                            if ((raft_num_peers = parse_cmdline_host_list(value, &raft_peers)) <= 0)
                                goto shit_usage;
                            break;
                        default:
                            goto shit_usage;
...
{
    return (void *)(((uintptr_t)memcpy(dest, src, n)) + n);
}*/
...
{
    //dbg_printf("     close_socket on fd %u\n", *fd);
    if (*fd != -1) {
...
#if 0
        if (global_raft_client_fd != -1)
            close_socket(&global_raft_client_fd);
#endif
    }
}
        ...
        const uint8_t uuid[const UUID_SIZE])
{
    ...
    raft_log_append(RAFT_LOG_REGISTER_TOPIC, ret->name, ret->uuid);
    pthread_rwlock_unlock(&global_topics_lock);
...
static int raft_new_conn(int new_fd, const struct sockaddr_in *sin, socklen_t /*sin_len*/)
{
    ...
    if ((rc = read(new_fd, hello_packet, sizeof(hello_packet))) != sizeof(hello_packet)) {
        ...
        warn("raft_new_conn: short read (%ld != %lu)", rc, sizeof(hello_packet));
        goto fail;
    }

    memcpy(&pck_type, hello_packet, sizeof(pck_type));
    pck_type = ntohs(pck_type);

    if (pck_type != RAFT_HELLO) {
        warn("raft_new_client: not RAFT_HELLO");
        errno = EINVAL;
        goto fail;
    }

    memcpy(&pck_len, hello_packet + 2, sizeof(pck_len));
    pck_len = ntohl(pck_len);

    if (pck_len != sizeof(hello_packet)) {
        warn("raft_new_client: pck_len is %u not %lu", pck_len, sizeof(hello_packet));
        errno = EFBIG;
        goto fail;
    }

    memcpy(&id, hello_packet + 2 + 4, sizeof(id));
    id = ntohl(id);

    type = hello_packet[2 + 4 + 4];
    ...
    switch (type)
    {
        case RAFT_PEER:
            if (raft_peers[idx].peer_fd > 0) {
                dbg_printf("RAFT raft_new_client: already connected on fd %u, closing\n",
        ...
        case RAFT_CLIENT:
            if (raft_state.clients[idx].fd > 0) {
                dbg_printf("RAFT raft_new_client: already_connected on fd %u, closing existing\n",
                        raft_state.clients[idx].fd);
                close_socket(&raft_state.clients[idx].fd);
                raft_active_clients--;
            }

            raft_state.clients[idx].fd = new_fd;
            raft_active_clients++;
            return idx;
...
static int raft_send(raft_conn_t mode, int target, raft_rpc_t rpc, ...)
{
    ...
    uint8_t arg_status, arg_conn_type;
    ...
    switch (rpc)
    {
        ...
        case RAFT_REQUEST_VOTE:
            ...
            arg_log_term = htonl(va_arg(ap, uint32_t));
            break;
        ...
        case RAFT_APPEND_ENTRIES:
            ...
            if (arg_num_entries) {
                ...
                {
                    length++; /* type */
                    length += 2 * sizeof(uint32_t); /* index and term */
                    switch (tmp->event)
                    {
    ...
    if ((ptr = packet_buffer = malloc(length)) == NULL)
        goto fail;

    packet_type = htons(rpc);
    packet_len = htonl(length);

    memcpy(ptr, &packet_type, 2); ptr += 2;
    memcpy(ptr, &packet_len, 4); ptr += 4;

    switch(rpc)
    {
        case RAFT_HELLO:
            memcpy(ptr, &arg_id, sizeof(arg_id));
            ptr += sizeof(arg_id);
            memcpy(ptr, &arg_conn_type, 1);
            ptr++;
            break;
        ...
        case RAFT_REQUEST_VOTE:
            ...
            memcpy(ptr, &arg_log_term, sizeof(uint32_t)); ptr += sizeof(uint32_t);
            break;
    ...
    if (target == -1) {
        ...
        if ((rc = write(*fd, packet_buffer, length)) != length) {
            ...
            //dbg_printf("RAFT raft_send: sent %lub to idx=%u (fd=%d)\n", length, target, raft_peers[target].peer_fd);
            sent = 1;
        }
    }
...
static void raft_change_to(raft_mode_t mode)
{
    ...
    raft_state.mode = mode;
}

static void raft_tick(void)
{
    int new_fd = 0;
    struct sockaddr_in sin;
    socklen_t sin_len;
    ...
    if (time(NULL) != last_run) {
        ...
        for (unsigned idx = 0; idx < raft_num_peers; idx++)
        {
            dbg_printf("RAFT raft_tick: idx[%2d] fd=%2u server_id=%2u addr=%08x:%u\n",
                    ...
                    ntohl(raft_peers[idx].address.s_addr),
                    ntohs(raft_peers[idx].port)
                    );
        }
#endif
    }
    ...
    for (unsigned idx = 0; idx < raft_num_peers; idx++)
    {
        if ( (idx == raft_state.self_id) ||
                    (raft_peers[idx].port == 0) ||
                    (raft_peers[idx].peer_fd > 0) )
            continue;
...
conn_skip:
    ...
    if (timems() > raft_state.election_timer) {
        switch (raft_state.mode)
        {
            case RAFT_FOLLOWER:
                if (raft_state.voted_for == 0)
                    raft_change_to(RAFT_CANDIDATE);
                break;
...
conn_fail:
    if (errno) {
        //dbg_printf("     raft_tick: conn_fail: %s\n", strerror(errno));
    }
    if (new_fd != -1) {
        //dbg_printf("     raft_tick: conn_fail: closing\n");
        close_socket(&new_fd);
    }
    goto conn_skip;
}
...
shit_packet:
    ...
    memcpy(&rpk_type, header, 2);
    memcpy(&rpk_size, header + 2, 4);

    rpk_type = ntohs(rpk_type);
    rpk_size = ntohl(rpk_size) - sizeof(header);

    //dbg_printf("RAFT raft_recv: type=%u length=%u\n", rpk_type, rpk_size);

    if ((ptr = packet = malloc(rpk_size)) == NULL)
        goto fail;
        ...
        case RAFT_APPEND_ENTRIES:
            {
                ...
                if (num_entries) {
                    ...
                    //dbg_printf("RAFT raft_recv: APPEND_ENTRIES: term=%u leader_id=%u prev_log_index=%u prev_log_term=%u leader_commit=%u PING\n",
                ...
                assert(raft_state.mode != RAFT_LEADER);
                ...
                if (raft_state.current_term > term)
                    reply = RAFT_FALSE;
                else
                ...
                /* of an existing entry conflicts, replace it and all
                 * subsequent entries */
        ...
        case RAFT_REQUEST_VOTE_REPLY:
            {
                ...
                dbg_printf("RAFT raft_recv: RAFT_REQUEST_VOTE_REPLY: %u voted %s\n",
                        sender, raft_status_str[status]);

                if (status == RAFT_TRUE)
                    raft_peers[sender].voted_for = raft_state.self_id;
                else
                    raft_peers[sender].voted_for = -1;

                unsigned votes = 0;
                const unsigned need = (raft_num_peers+1)/2;
                const unsigned total = (raft_num_peers+1);

                for (unsigned idx = 0; idx < raft_num_peers; idx++)
                    if (raft_peers[idx].voted_for == raft_state.self_id)
                        votes++;

                if (raft_state.self_id == raft_state.voted_for)
                    votes++;

                dbg_printf("RAFT raft_recv: RAFT_REQUEST_VOTE_REPLY: i have %u/%u votes and need %u\n",
                        votes, total, need);

                if (votes >= need)
                    raft_change_to(RAFT_LEADER);
            }
            break;

        default:
            warnx("raft_recv: unknown type: %s", raft_rpc_str[rpk_type]);
    }
...
static void raft_clean(void)
{
    ...
    if (raft_state.clients)
        free(raft_state.clients);
}
...
static int raft_log_append(raft_log_t event, ...)
{
    ...
    prev_log = raft_state.log_head;

    new_log->event = event;
    new_log->term = raft_state.current_term;
    new_log->index = ++raft_state.last_applied;
...
static RETURN_TYPE main_loop(void *start_args)
{
    ...
    while (running)
    {
        int rc = 0;
        struct timeval tv;
        int max_fd = MAX(mother_fd, om_fd);
        ...
        */
        ...
        if (has_clients == false) {
            tv.tv_usec = opt_raft ? 20000 : 1000000;
...
#if 0
        if (raft_client_fd > 0) {
            if (FD_ISSET(raft_client_fd, &fds_in)) {
                int tmp_fd;
                if ((tmp_fd = accept(raft_client_fd, NULL, NULL)) != -1) {
                    dbg_printf("RAFT main_loop: accept.client fd %u\n", tmp_fd);
                    if (raft_new_conn(RAFT_CLIENT, tmp_fd) == -1) {
                        dbg_printf("RAFT main_loop: raft_new_client: failed: %s\n", strerror(errno));
                        close_socket(&tmp_fd);
                    }
                } else
                    warn("main_loop: raft.client accept failed");
            }
        }
#endif
...
int main(int argc, char *argv[])
{
    ...
    //opt_raft_client_listen.s_addr = htonl(INADDR_LOOPBACK);
    ...
    if (opt_raft) {
        ...
        //if ((global_raft_client_fd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) == -1)
        //    logger(LOG_EMERG, NULL, "socket(raft.client): %s", strerror(errno));
        sock_linger(global_raft_fd);
        sock_reuse(global_raft_fd, 1);
        sock_nonblock(global_raft_fd);
        /*sock_linger(global_raft_client_fd);
        sock_reuse(global_raft_client_fd, 1);
        sock_nonblock(global_raft_client_fd);*/
    }
...
#if 0
        sin.sin_family = AF_INET;
        sin.sin_port = htons(opt_raft_client_port);
        sin.sin_addr.s_addr = opt_raft_client_listen.s_addr;
        inet_ntop(AF_INET, &sin.sin_addr, bind_addr, sizeof(bind_addr));
        logger(LOG_NOTICE, NULL, "main: raft binding client to %s:%u",
                bind_addr, opt_raft_client_port);

        if (bind(global_raft_client_fd, (struct sockaddr *)&sin, sizeof(sin)) == -1)
            logger(LOG_EMERG, NULL, "bind(raft.client): %s", strerror(errno));

        if (listen(global_raft_client_fd, 5) == -1)
            logger(LOG_EMERG, NULL, "listen(raft.client): %s", strerror(errno));
#endif
    ...
    struct start_args start_args_raft = {
        ...
        //.raft_client_fd = opt_raft ? global_raft_client_fd : 0,
    };
#else
