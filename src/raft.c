...
[[maybe_unused]] static int raft_save_state(void)
{
    int state_fd = -1;
    ...
    const uint32_t save_term = raft_state.log_tail->term;
    const uint32_t save_index = raft_state.log_tail->index;

    if ((state_fd = open("state_save.new", O_WRONLY|O_CREAT|O_TRUNC|O_SYNC, 0600)) == -1) {
        warn("raft_save_state: open");
        goto fail;
    }

    if (write(state_fd, &save_term, sizeof(uint32_t)) != sizeof(uint32_t)) {
        warn("raft_save_state: write");
        goto fail;
    }

    if (write(state_fd, &save_index, sizeof(uint32_t)) != sizeof(uint32_t)) {
        warn("raft_save_state: write");
        goto fail;
    }
    ...
    for (struct raft_log *p = raft_state.log_head; p; p = p->next)
    {
        ...
        switch(p->event)
        {
            case RAFT_LOG_REGISTER_TOPIC:
                rdbg_cprintf("<%s> uuid=%s ",
                        p->register_topic.name,
                        uuid_to_string(p->register_topic.uuid)
                        );
                if ( (p->register_topic.flags & RAFT_LOG_REGISTER_TOPIC_HAS_RETAINED) )
                    rdbg_cprintf("msg_uuid=%s ", uuid_to_string(p->register_topic.msg_uuid));
                break;
    ...
    /* Debugging as save state is incomplete */
    abort();
...
static struct raft_log *raft_alloc_log(raft_conn_t conn_type, raft_log_t event)
{
    ...
    if (conn_type == RAFT_CLIENT) {
        ...
        if (pthread_cond_init(&ret->cv, NULL) != 0)
            goto fail;
        else
            cond_init = true;
    }
...
static int raft_free_log(struct raft_log *entry)
{
    ...
    {
        case RAFT_LOG_REGISTER_TOPIC:
            if (raft_impl->free_log)
                raft_impl->free_log(entry);
            break;
...
static int raft_commit_and_advance(void)
{
    ...
    {
        case RAFT_LOG_REGISTER_TOPIC:
            if (raft_impl->commit_and_advance)
                if(raft_impl->commit_and_advance(log_entry) == -1)
                    goto fail;
            break;
...
static int raft_await_client_response(struct raft_log *pending_event)
{
    int rc;
    struct timespec ts;
    clock_gettime(CLOCK_REALTIME, &ts); /* TODO convert to CLOCK_MONOTONIC to avoid timejumps */
    ts.tv_sec += 5;
...
int raft_send(raft_conn_t mode, struct raft_host_entry *client, raft_rpc_t rpc, ...)
{
    ...
    switch (rpc)
    {
        ...
        case RAFT_CLIENT_REQUEST:
                ...
                case RAFT_LOG_REGISTER_TOPIC:
                    {
                        ...
                        if (raft_impl->pre_send)
                            if (raft_impl->pre_send(arg_event, &send_state) == -1)
                                goto fail;
                    }
                    break;
            ...
            send_state.arg_req_len = arg_req_len;
            ...
            if (arg_num_entries) {
                ...
                for (idx = 0; tmp && idx < arg_num_entries; idx++)
                {
                    ...
                    switch (tmp->event)
                    {
                        case RAFT_LOG_REGISTER_TOPIC:
                            if (raft_impl->pre_send)
                                if (raft_impl->pre_send(tmp, &send_states[idx]) == -1)
                                    goto fail;
                            break;
            ...
            *ptr++ = arg_req_flags;
            ...
            switch ((raft_log_t)arg_req_type)
            {
                case RAFT_LOG_REGISTER_TOPIC:
                    ...
                    if (raft_impl->fill_send)
                        if (raft_impl->fill_send(&send_state, arg_event) == -1)
                            goto fail;
                    ...
                    switch(tmp->event)
                    {
                        case RAFT_LOG_REGISTER_TOPIC:
                            ...
                            if (raft_impl->fill_send) {
                                ...
                                if ((rc = raft_impl->fill_send(&send_states[idx], tmp)) == -1)
                                    goto fail;
                                entry_length += rc;
...
send_client_request_reply:
        ...
        case RAFT_APPEND_ENTRIES_REPLY:
            {
                ...
                if (client_term > raft_state.current_term) {
                    rdbg_printf("RAFT raft_recv: RAFT_APPEND_ENTRIES_REPLY has higher term from id=%u, converting\n",
                            client->server_id);
        ...
        default:
            warnx("raft_recv: unknown type: %d", rpc);
            goto fail;
    }
