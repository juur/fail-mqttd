...
//static const unsigned  RAFT_MAX_LOG_LENGTH   = 10;
...
[[gnu::nonnull(3),gnu::format(printf,3,4)]] void logger(int priority, const struct client *client, const char *format, ...);
...
static int raft_append_log(struct raft_log *entry, struct raft_log **head, struct raft_log **tail, _Atomic long *log_len);
...
static int raft_save_state(bool header_only)
{
    ...
    if ((state_fd = open(file_name, open_flags, 0600)) == -1) {
        warn("raft_save_state: open");
        goto fail;
    }
    ...
    for (struct raft_log *p = raft_state.log_head; p; p = p->next)
    {
        ...
        if ((event_buf_len = raft_impl->handlers[p->event].save_log(p, &event_buf)) == -1) {
            warn("raft_save_state: save_log");
            goto fail;
        }

        if (event_buf_len < 0 || (size_t)event_buf_len > USHRT_MAX) {
            warnx("raft_save_state: event_buf_len is %d", event_buf_len);
            errno = EOVERFLOW;
            goto fail;
        }
        ...
        if (event_buf) {
            if (write(state_fd, event_buf, event_buf_len) != event_buf_len) {
                warn("raft_save_state: write(event_buf)");
                goto fail;
            }
            free(event_buf);
            event_buf = NULL;
...
static int raft_load_state(void)
{
    ...
    rdbg_printf("RAFT raft_load_state: start\n");
    ...
    snprintf(file_name, BUFSIZ, "save_state.%u", raft_state.self_id);

    if ((state_fd = open(file_name, O_RDONLY)) == -1) {
        warn("raft_load_state: open");
        goto fail;
    }
...
static int raft_add_write(struct raft_host_entry *client, const uint8_t *buffer, ssize_t size)
{
    ...
    client->wr_packet_buffer = (void *)buffer;
...
static int raft_try_write(struct raft_host_entry *client)
{
    ...
    pthread_rwlock_wrlock(&client->wr_lock);
    if (client->wr_packet_buffer == NULL || client->peer_fd == -1) {
        pthread_rwlock_unlock(&client->wr_lock);
        errno = EINVAL;
        return -1;
    }
    ...
    if ((rc = write(client->peer_fd, client->wr_packet_buffer + client->wr_offset, client->wr_need)) == client->wr_need) {
        raft_reset_write_state(client, false);
        pthread_rwlock_unlock(&client->wr_lock);
        return 0;
    }
...
int raft_send(raft_conn_t mode, struct raft_host_entry *client, raft_rpc_t rpc, ...)
{
    ...
    if (mode == RAFT_PEER && client != NULL && client->peer_fd == -1) {
        ...
        warnx("raft_send: RAFT_SERVER without client or peer_fd");
        errno = EBADF;
        return -1;
    }
    ...
    switch (rpc)
    {
        ...
        case RAFT_APPEND_ENTRIES:
            ...
            if (packet.length < (RAFT_HDR_SIZE + RAFT_APPEND_ENTRIES_FIXED_SIZE)) {
                warn("raft_send: RAFT_APPEND_ENTRIES: packet.length too short");
                goto fail;
            }
...
static int raft_tick(void)
{
    ...
    switch(raft_state.state)
    {
        case RAFT_STATE_NONE:
            warnx("raft_tick: RAFT_STATE_NONE");
            abort();
            break;
        ...
        case RAFT_CLIENT_REQUEST_REPLY:
            {
                ...
                warn("raft_process_packet: RAFT_CLIENT_REQUEST_REPLY: got an error: <%s>",
        ...
        case RAFT_REQUEST_VOTE:
            {
                ...
                if (term > raft_state.current_term) {
                    rdbg_printf("RAFT raft_process_packet: higher term received from id=%u\n",
                            client->server_id);
                    raft_update_term(term);
                    raft_change_to(RAFT_STATE_FOLLOWER);
                    raft_stop_election();
        ...
        case RAFT_APPEND_ENTRIES_REPLY:
            {
                ...
                if (client_term > raft_state.current_term) {
                    rdbg_printf("RAFT raft_process_packet: RAFT_APPEND_ENTRIES_REPLY has higher term from id=%u, converting\n",
                            client->server_id);
                ...
                } else if (raft_state.log_head && client->next_index > raft_state.log_head->index) {
                    if (client->next_index > 1)
                        client->next_index--;
                ...
                //rdbg_printf("RAFT raft_process_packet: APPEND_ENTRIES_REPLY %s from %u\n",
        ...
        case RAFT_REQUEST_VOTE_REPLY:
            {
                ...
                if (term > raft_state.current_term) {
                    rdbg_printf("RAFT raft_process_packet: RAFT_REQUEST_VOTE_REPLY has higher term from id=%u, converting.\n",
                            client->server_id);
                    raft_update_term(term);
                    raft_change_to(RAFT_STATE_FOLLOWER);
                ...
                rdbg_printf("RAFT raft_process_packet: RAFT_REQUEST_VOTE_REPLY: %u voted %s\n",
                ...
                rdbg_printf(BRED "RAFT raft_process_packet: RAFT_REQUEST_VOTE_REPLY: %d/%d votes. %d voted. need %d" CRESET "\n",
                        votes, total, has_voted, need);

                if (votes >= need) {
                    rdbg_printf(BYEL "RAFT raft_process_packet: RAFT_REQUEST_VOTE_REPLY: i have won!" CRESET "\n");
...
# endif
    ...
    if (raft_load_state() == -1) {
        warnx("raft_init: raft_load_state");
        if (errno != ENOENT)
            return -1;
    }
